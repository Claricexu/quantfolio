...
│
├── tests/
│   ├── unit/                     # ~159 plain-assert tests across 17 modules (see §8)
...
- **Forensic-flag layer ([`fundamental_screener._compute_forensic_flags`](fundamental_screener.py), Round May 15):** A separate column family from the verdict — adds `forensic_flags_json` + `forensic_flag_count` to `screener_results.csv` without touching the LEADER/WATCH/AVOID/INSUFFICIENT_DATA verdict logic (Round 9a's "verdicts encode pure quality" invariant is preserved). Three working flags ship: `ni_ocf_divergence`, `leverage_high`, `dilution_velocity`. The fourth (`going_concern`) is a **stub that always returns False** because SEC files going-concern as narrative text in 10-K Item 8 / Auditor's Report rather than as a structured XBRL fact — empirical probe of nine known going-concern filers found zero exposure under the companyfacts API. The schema slot ships anyway; a future 10-K text-parser drops in without consumer changes (stub status documented inline at `fundamental_screener.py:_flag_going_concern` and `fundamental_metrics.py:going_concern_present`). Sector exclusion: SIC 6000-6799 (banks/insurance/REITs) — the flagged behaviours are normal under their accounting frameworks.
  - **Override path:** `cache/forensic_flag_overrides.csv` (schema `symbol,flag_name,expires_at,reason`). Loaded once per process via `_load_forensic_flag_overrides`; cached at module level under `_FORENSIC_OVERRIDES_CACHE`. Quarterly rebuild cadence makes the read-once contract race-free; tests can call `reset_forensic_overrides_cache()` to swap fixtures. Suppressed flags stay True in `forensic_flags_json` (the frontend renders them as dashed-border / muted "overridden" chips) but are excluded from `forensic_flag_count`, which is the leader-pool gate. Single source of truth per PATTERNS.md P-4.
  - **Pool filter:** `leader_selector.py` drops any LEADER row with `forensic_flag_count > 0` from `leaders.csv` (`_has_active_forensic_flags`). Legacy rows from a pre-Round-May-15 `screener_results.csv` (no `forensic_flag_count` cell) pass through as if zero — graceful degradation. The selector stashes the dropped-symbol list on `select_leaders._last_forensic_dropped` so the under-fill log can distinguish forensic-driven shrinkage from genuinely-too-few-LEADERs.
  - **Frontend:** `frontend/index.html` renders amber chips on the Verdict Card alongside the existing red dealbreaker chips (reds first, ambers second per sophia's locked design). Dismissible legend strip above the Leader Detector table; localStorage key `legendForensicChipsDismissed` persists the dismissal. `verdict_provider._coerce_row` deserializes `forensic_flags_json` (per-flag enriched shape `{flag: {value, suppressed}}`) and adds `forensic_flag_count` to `_INT_COLS`.
...
| `test_forensic_flags.py` | 27 | Round May 15 forensic-flag layer: ni_ocf_divergence streak semantics, leverage_high AND threshold + missing inputs, dilution_velocity strict-`>` 10% boundary, SIC 6000-6799 sector exclusion, override active/expired/cross-ticker/cross-flag/missing-CSV/comments-only, going-concern stub passthrough + schema-slot presence + override CSV forward-compat |
| `test_leader_selector.py` | 10 | `_has_active_forensic_flags` semantics on numeric/blank/garbage cells, pool-filter integration (clean LEADER passes / forensic-flagged LEADER excluded / legacy blank cell passes through), `_last_forensic_dropped` exposure for under-fill log, dealbreaker independence, non-LEADER skip (Round May 15) |
...
- **Going-concern detection (Round May 15 deferred):** `_flag_going_concern` is a stub that always returns False. SEC files going-concern as narrative text in 10-K Item 8 (Auditor's Report), not as a structured XBRL fact — empirical probe of nine known going-concern filers found zero exposure under SEC's companyfacts API. Closing this gap requires a 10-K text-parsing pipeline that reads the Auditor's Report (out of scope for May 15). The schema slot ships anyway: column exists in `forensic_flags_json`, override CSV accepts `flag_name=going_concern`, frontend has chip label + tooltip wired in. Forward-compat: when text-parsing lands, no consumer changes are required. Tracked in [FEATURE_BACKLOG.md](FEATURE_BACKLOG.md).
