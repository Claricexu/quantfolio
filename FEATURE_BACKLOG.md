...
- **FB-1 (display half)** — Verdict card now shows peer-median column alongside each company value (8 metrics, bucketed by industry_group, ≥5 non-null members threshold). Three-column CSS grid layout (label | company | peer median). Inline note for ETFs and em-dash on categorical rows. Shipped Round 7d (commits e3555a2, 250aa07, 301b94a, 0076b0e, a1ae043).
- **FB-4** — Verdict card data-freshness chip + ETF P/E tooltip + verdict-card layout refactor (the cluster carried over from Round 7c-2's deferred list). Timestamp chip beside SCORE box uses shared `.as-of-chip` style; hover reveals raw ISO mtime. ETF P/E tooltip "Weighted average of holdings" on the Compare card. Layout refactor is the same three-column grid that hosts FB-1's display half. Shipped Round 7d (commits 0076b0e, 10be25b, a1ae043).